...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("plot_type1", "Select Plot Type for Plot 1", choices = c("Density Plot", "Box Plot", "Violin Plot")),
      selectInput("plot_type2", "Select Plot Type for Plot 2", choices = c("Bar Plot", "Line Plot")),
...
server <- function(input, output, session) {
  ...
  output$plot1 <- renderPlot({
    ...
    p <- ggplot(plot_data, aes(x = average_HDI, fill = country_name)) +
      theme_minimal() + labs(title = "Average HDI Density Plot", x = "Average HDI", y = "Density")
    
    if (input$plot_type1 == "Density Plot") {
      p <- p + geom_density(alpha = 0.7)
    } else if (input$plot_type1 == "Box Plot") {
      p <- ggplot(plot_data, aes(x = country_name, y = average_HDI, fill = country_name)) +
        geom_boxplot() + theme_minimal() + labs(title = "Average HDI Box Plot", x = "Country", y = "Average HDI")
    } else if (input$plot_type1 == "Violin Plot") {
      p <- ggplot(plot_data, aes(x = country_name, y = average_HDI, fill = country_name)) +
        geom_violin() + theme_minimal() + labs(title = "Average HDI Violin Plot", x = "Country", y = "Average HDI")
    }
    
    p + scale_fill_brewer(palette = input$color_palette)
  })
  ...
  output$plot2 <- renderPlot({
    ...
    p <- ggplot(plot_data, aes(x = year, y = mean_value, fill = country_name)) +
      theme_minimal() + labs(title = "Gender Inequality Index", x = "Year", y = "Mean Value")
    
    if (input$plot_type2 == "Bar Plot") {
      p <- p + geom_bar(stat = "identity", position = "dodge", na.rm = TRUE) + ylim(y_limits)
    } else if (input$plot_type2 == "Line Plot") {
      p <- p + geom_line(aes(color = country_name, group = country_name)) + geom_point() + ylim(y_limits)
    }
